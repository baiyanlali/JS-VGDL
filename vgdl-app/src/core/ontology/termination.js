...
export class SpriteCounter extends Termination{
	...
	if (game.numSprites(this.stype) <= this.limit) {
		...
		console.debug("Termination Result:",this.stype, game.numSprites(this.stype), "/", this.limit, this.win);
		return [true, this.win];
	}
	else
		return [false, null];
...
export class MultiSpriteCounter extends Termination{
	constructor(args) {
		...
		this.stypes = this.array_args.filter(arg => {return arg.includes('stype')}).map(stype => {return args[stype]})
	}
